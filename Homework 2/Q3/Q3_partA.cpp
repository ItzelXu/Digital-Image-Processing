...
void getshrinking(float** rPixelfloat, int imageHeight, int imageWidth,int** M, float** routputArray,int* countArray)
{
    ...
    int bond=0;
    string str;
    for(iterationIndex=1;iterationIndex<15;iterationIndex++)
    {
        count=0;
        ...
        //To count number of stars shrinking in every iteration
        for(rowIndex=0;rowIndex<imageHeight;rowIndex++)
        {
            for (colIndex=0;colIndex<imageWidth;colIndex++)
            {
        ...
        if(count-previousCount!=0)
        {
            ...
            cout<<"Count is: "<<countArray[iterationIndex]<<endl<<endl;
        }
        previousCount=count;
    }
}

int getstars(unsigned char** rpixelshrinked,int imageHeight, int imageWidth)
{
    ...
    for(rowIndex=0;rowIndex<imageHeight;rowIndex++)
    {
        for(colIndex=0;colIndex<imageWidth;colIndex++)
        {
            if(rpixelshrinked[rowIndex][colIndex]==255) // To count number of stars shrinking to a single white dot in a black background
            {
                count++;
            }
        }
...

{
    ...
    //To count total number of stars
    int count=getstars(rpixel2Dshrinked,imageHeight,imageWidth);
    cout<<"Total number of stars are: "<<endl<<count<<endl;
